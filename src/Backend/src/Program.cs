            ...
            using IHost host = Host.CreateDefaultBuilder(args)
                .ConfigureServices((hostBuilderContext, services) =>
                {
                    if (!hostBuilderContext.HostingEnvironment.IsEnvironment("IntegrationTesting"))
                    {
                        services.AddCors(options =>
                        {
                            options.AddPolicy("devCorsPolicy",
                                policy =>
                                {
                                    ...
                                    policy.WithOrigins(new string[] { "http://127.0.0.1:60006" });
                                    policy.AllowAnyHeader();
                                    policy.AllowAnyMethod();
                                    policy.AllowCredentials(); // Added this with cors policy change
                                });
                        });
                        ...
                        services.AddSingleton<IWorkloadClientService, WorkloadClientService>();

                        // Access to Fabric Core
                        services.AddSingleton<ICoreClientService, CoreClientService>();

                        // Access to Fabric Eventhouse
                        services.AddSingleton<IEventhouseClientService, EventhouseClientService>();

                        // Access to Fabric Data Pipeline
                        services.AddSingleton<IDataPipelineClientService, DataPipelineClientService>();

                        // Workloads
                        services.AddSingleton<IInvestmentInsightsClientService, InvestmentInsightsClientService>();
