...
namespace Boilerplate.Services
{
    ...
    public class LakehouseClientService : ILakehouseClientService
    {
        ...
        /// <summary>
        /// Retrieves a list of Lakehouses available in the selected Workspace using the provided bearer token.
        /// </summary>
        /// <param name="token">The access token required to authorize the API requests.</param>
        /// <param name="workspaceId">The id of the workspace that contains the directory.</param>
        /// <returns>A list of Lakehouses</returns>
        public async Task<Lakehouses> GetLakehouseList(string token, Guid workspaceId)
        {
            // Create the URL using the provided source
            string url = $"{EnvironmentConstants.FabricApiBaseUrl}/v1/workspaces/{workspaceId}/lakehouses";
            ...
            try
            {
                ...
                // Read the response content as a string
                var content = await response.Content.ReadAsStringAsync();
                var lakehouses = JsonConvert.DeserializeObject<Lakehouses>(content);
        ...
        /// <summary>
        /// Create a lakehouse in the provided Workspace using the provided bearer token.
        /// </summary>
        /// <param name="token">The access token required to authorize the API requests.</param>
        /// <param name="workspaceId">The id of the workspace that contains the directory.</param>
        /// <param name="lakehouseName">The name of the lakehouse to be created.</param>
        /// <param name="lakehouseDescription">The description of the lakehouse to be created.</param>
        /// <returns>Response from the lakehouse creation</returns>
        public async Task<ItemCreateResponse> CreateLakehouse(string token, Guid workspaceId, string lakehouseName, string lakehouseDescription)
        {
            // Create the URL using the provided source
            string url = $"{EnvironmentConstants.FabricApiBaseUrl}/v1/workspaces/{workspaceId}/lakehouses";

            var lakehouseSettings = new { 
                displayName = lakehouseName, 
                description = lakehouseDescription 
            };
            var postContent = new StringContent(JsonConvert.SerializeObject(lakehouseSettings), Encoding.UTF8, "application/json");
